...
export default function PaymentEntryPage() {
  ...
  const [sessionPayments, setSessionPayments] = useState<Voucher[]>([])

  const handleFormSubmit = (data: VoucherFormValues) => {
    // For payments, total amount is the debit side
    ...
    setSessionPayments(prev => [newVoucher, ...prev])
  ...
  const totalSessionPayment = useMemo(() => 
    sessionPayments.reduce((acc, curr) => acc + curr.totalAmount, 0)
  , [sessionPayments])
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-medium flex items-center gap-2">
            ...
            Record cash or bank payments made to vendors, staff, or others.
          </p>
        </div>

        <div className="flex items-center gap-4">
           <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-right">
              <p className="text-[10px] font-black uppercase text-rose-700 tracking-widest">Payments This Session</p>
              <p className="text-2xl font-black font-mono text-rose-600">₹{totalSessionPayment.toLocaleString()}</p>
           </div>
        </div>
      </div>
        ...
        <Card className="xl:col-span-12 rounded-[2.5rem] border-border/40 bg-card/20 backdrop-blur-xl shadow-2xl overflow-hidden">
          <CardContent className="p-8">
            <div className="flex items-center gap-3 mb-8 px-2">
               ...
               <h2 className="text-xl font-bold">New Payment Voucher</h2>
            </div>
        ...
        <div className="xl:col-span-12 space-y-6">
           <div className="flex items-center justify-between px-2">
              ...
              <Badge variant="outline" className="rounded-full bg-primary/5 text-primary border-primary/20">
                 {sessionPayments.length} Entries Posted
              </Badge>
           </div>

           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {sessionPayments.length > 0 ? (
                sessionPayments.map((voucher) => (
                    ...
                    <CardContent className="p-6">
                       <div className="flex justify-between items-start mb-4">
                          ...
                          <div className="text-right">
                             <p className="text-xl font-black font-mono text-rose-500">₹{voucher.totalAmount.toLocaleString()}</p>
                       ...
                       <div className="space-y-2 mb-4">
                          {voucher.lines.filter(l => l.drAmount > 0).map((l, idx) => (
                             <div key={idx} className="flex justify-between items-center text-[11px] font-bold">
                                <span className="text-muted-foreground">Paid To: {getLedgerName(l.ledgerId)}</span>
                ...
                <div className="col-span-full p-20 rounded-[3rem] border-2 border-dashed border-border/20 flex flex-col items-center justify-center text-center gap-4 text-muted-foreground opacity-30">
                   ...
                   <div className="space-y-1">
                      ...
                      <p className="text-xs font-medium">Post a new payment to see it in history.</p>
                   </div>
                </div>
              )}
           </div>
