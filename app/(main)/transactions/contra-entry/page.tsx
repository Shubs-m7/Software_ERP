...
import { History, Landmark, Building2, Repeat } from "lucide-react"
import { cn } from "@/lib/utils"

export default function ContraEntryPage() {
  const { addVoucher } = useAppStore()
  const [sessionContras, setSessionContras] = useState<Voucher[]>([])
  ...
  const handleFormSubmit = (data: VoucherFormValues) => {
    ...
    setSessionContras(prev => [newVoucher, ...prev])
    alert("Contra Voucher posted successfully!")
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-medium flex items-center gap-2">
            ...
            Internal transfers between Cash and Bank accounts.
          </p>
        </div>

        <div className="flex items-center gap-4">
           <div className="p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-right">
              <p className="text-[10px] font-black uppercase text-blue-700 tracking-widest">Contra Entries</p>
              <p className="text-2xl font-black font-mono text-blue-600">{sessionContras.length} Posted</p>
           </div>
        </div>
      </div>
        ...
        <Card className="xl:col-span-12 rounded-[2.5rem] border-border/40 bg-card/20 backdrop-blur-xl shadow-2xl overflow-hidden">
          <CardContent className="p-8">
            <div className="flex items-center gap-3 mb-8 px-2">
               ...
               <h2 className="text-xl font-bold">New Contra Voucher</h2>
            </div>
        ...
        <div className="xl:col-span-12 space-y-6">
           <div className="flex items-center justify-between px-2">
              <div className="flex items-center gap-3">
                 ...
                 <h2 className="text-lg font-bold">Recent Contras</h2>
              </div>
           </div>

           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {sessionContras.length > 0 ? (
                sessionContras.map((voucher) => (
                    ...
                    <CardContent className="p-6">
                       ...
                       <div className="space-y-3 mb-4">
                          {voucher.lines.map((l, idx) => (
                             <div key={idx} className="flex justify-between items-center text-[10px] font-bold">
                                <span className={cn(l.drAmount > 0 ? "text-blue-600" : "text-muted-foreground")}>
                       ...
                       <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-2">
                             ...
                             <span className="text-[10px] font-black uppercase tracking-tighter text-blue-600">Fund Transfer</span>
                          </div>
                          {voucher.narration && (
                ...
                <div className="col-span-full p-20 rounded-[3rem] border-2 border-dashed border-border/20 flex flex-col items-center justify-center text-center gap-4 text-muted-foreground opacity-30">
                   ...
                   <div className="space-y-1">
                      <p className="text-sm font-black uppercase tracking-widest text-primary">No Transfers Found</p>
                      <p className="text-xs font-medium">Record a cash/bank transfer to see it in history.</p>
                   </div>
                </div>
              )}
           </div>
